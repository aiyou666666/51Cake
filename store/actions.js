...
export default {
  async nuxtServerInit({ dispatch, commit, getters }, { req, res }) {
    if (req.headers.cookie) {
      ...
      if(cookieObj.githubToken){
	    ...
	    const { data } = await axios.get(`${getters.baseUrl}/user?token=${cookieObj.token}`)
  ...
  //用户注册
  async REGIST({ commit, state, getters }, user){
  	const { data } =await axios.post(`${getters.baseUrl}/regist`,user)
  	commit('SET_TOKEN', data.data.token)
    return data
